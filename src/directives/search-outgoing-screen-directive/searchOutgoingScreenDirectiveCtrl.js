    ...
    app.controller('searchOutgoingScreenDirectiveCtrl', function (searchOutgoingService,
        ...
        function getClassificationsByDocumentClass(classifications) {
            ...
            return classifications.filter(function (classification) {
                return classification.classification.docClassId === null || classification.classification.docClassId === 0
            });
        }
                ...
                .then(function () {
                    self.controllerService
                        ...
                        .then(function (result) {
                            ...
                            self.grid.limitOptions = gridService.getGridLimitOptions(gridService.grids.search.outgoing, self.searchedOutgoingDocuments);
                            self.selectedSearchedOutgoingDocuments = [];
                            self.selectedTab = 1;
                        })
         ...
         * @param defer
         */
        self.launchDistributionWorkflow = function (searchedOutgoingDocument, $event, defer) {
        ...
        self.endFollowupBulk = function ($event) {
            correspondenceService
                .endCorrespondenceFollowupBulk($event, self.selectedSearchedOutgoingDocuments)
                .then(function () {
                    self.reloadSearchCorrespondence(self.grid.page)
                        .then(function () {
                        })
        ...
        self.canEndFollowupBulk = function () {
            ...
            return _.every(self.selectedSearchedOutgoingDocuments, function (document) {
        ...
        self.previewDocument = function (searchedOutgoingDocument) {
            ...
            correspondenceService.viewCorrespondence(searchedOutgoingDocument, self.gridActions, checkIfEditPropertiesAllowed(searchedOutgoingDocument, true), checkIfEditCorrespondenceSiteAllowed(searchedOutgoingDocument, true))
                .then(function () {
                    self.reloadByCorrespondence(searchedOutgoingDocument);
                })
                .catch(function () {
                    self.reloadByCorrespondence(searchedOutgoingDocument);
                });
        ...
        self.gridActions = [
            ...
            // Launch Sequential Workflow
            {
                ...
                checkShow: function (action, model) {
                    return rootEntity.hasPSPDFViewer() && !model.hasActiveSeqWF() && !model.isCorrespondenceApprovedBefore() && !model.isCompositeSites() && !model.isBroadcasted() && !model.isTerminatedSEQ();
                }
            },
            // View Seq WF Steps
            {
